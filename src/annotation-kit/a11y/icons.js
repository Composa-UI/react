 ...
 * ./icons/<category>/; these are the 6 representative ones wired to our current types. The source
 * SVGs are white-filled (for coloured pills) — we swap white → currentColor so the same icon can
 * render white on a pill and type-coloured in the detail card, and normalise the box to 13px.
 *
 * NB `?raw` is a Vite/Storybook feature (Composa's builder). A bundler-agnostic build would inline
 * these strings instead. The other 31 icons (Form subtypes, List, Landmark variants, Media) are
 * copied in ./icons/ for when the type taxonomy expands.
 */
